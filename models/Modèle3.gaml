...
* Name: Modele3
* Author: Madaliou
* Description: Simulation d'un parking(approche 3)
* 
*/
     ...
     aspect base{
     	...
     	reflex goEnter when: aim="Enter" and target!=nil{
     		...
     		if(self.location=target){
     			target <- nil;	
     		list neighbour <- list(place) where (each.state="Free" and each.type="petit" );
     		list neighbour1 <- list(place) where (each.type="large" and each.state="Free");
     		list neighbour2 <- list(place) where (each.type="medium" and each.state="Free");
	...
	output {
		...
		display My_chart refresh:every(3600#s){
         chart "diagram" type:series size: {1, 0.5} position: {0, 0}{
             data "yield_per_hour" value:yield style: line color: #green ;
             data "availablePlace_per_hour" value:availablePlaceTo style: line color: #red ;
